<script lang="ts" context="module">
	...
	export async function load({ page }) {
		...
		const tracklists = await api.get(`/tracklists?verbose=1&skip=${skip}&query=${query}`);
...
<div class="flex flex-row">
	{#if pageNumber > 1}
		<ButtonLink href="/search?query={query}&page={pageNumber - 1}" text="Previous" />
	{:else}
		<ButtonLink text="Previous" />
	{/if}

	<ButtonLink href="/search?query={query}&page={pageNumber + 1}" text="Next" />
</div>
